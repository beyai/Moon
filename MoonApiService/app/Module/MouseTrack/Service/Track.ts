...
// 轨迹数据
type Track = [ x: number, y: number, t: number ]
// 分析结果
 ...
 * @returns - 解析后的数字
 */
function parsetST(str: string, charSet: string[]): number {
    let value = 0;
 ...
 */
function decrypt(encryptedStr: string): Track[] {
    const charSet = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-~".split("");
    const result:Track[] = [];
    let index = 0;

    // 解析第一个点（无符号位，长度3+4+7）
    const firstVal1 = parsetST(encryptedStr.substring(index, index + 3), charSet);
    index += 3;
    const firstVal2 = parsetST(encryptedStr.substring(index, index + 4), charSet);
    index += 4;
    const firstVal3 = parsetST(encryptedStr.substring(index, index + 7), charSet);
    index += 7;
    ...
    while (index < encryptedStr.length) {
        ...
        const num1 = parsetST(diff1Str.substring(1), charSet);
        ...
        const num2 = parsetST(diff2Str.substring(1), charSet);
        const diff2 = sign2 * num2;

        // 解析第三个差值（4位：无符号）
        const diff3 = parsetST(encryptedStr.substring(index, index + 4), charSet);
        index += 4;
...
function analyzeTrack(track: Track[]): AnalyzeResult {
    ...
    let totalTime = 0;
    let speedVariationsX: number[] = []; // X轴速度变化
    let angleChanges: number[] = []; // 整体角度变化
    let yFluctuations: number[] = []; // Y轴波动
...
/**
 * 验证操作是否为人类操作
 * @param encryptedStr 轨迹密文数据
 * @returns 
 */
export function checkTrack(encryptedStr: string): boolean {
    if (!encryptedStr || typeof encryptedStr !== 'string') {
        throw new Error(`数据格式不正确`)
    }
    const trackData = decrypt(encryptedStr)
    const result = analyzeTrack(trackData)
